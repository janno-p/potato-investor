use eventstore::{
    Connection, OnEventAppeared, ResolvedEvent, SubscriptionConsumer, SubscriptionEnv,
};
...
impl EventConsumer {
    pub fn new(eventstore: &Connection, sender: SyncSender<Event>) -> JoinHandle<EventConsumer> {
        let subscription = eventstore.subscribe_to_stream_from("bank-stream").execute();

        spawn(move || subscription.consume(EventConsumer { sender }))
    }
}
